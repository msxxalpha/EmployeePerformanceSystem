...
[HttpPost][ValidateAntiForgeryToken]public async Task<IActionResult> Login(LoginVm m){
 var u=await db.Users.SingleOrDefaultAsync(x=>x.UserName==m.UserName&&x.IsActive);
 ...
 if(u.EmployeeId.HasValue){var e=await db.Employees.FindAsync(u.EmployeeId.Value);if(e==null||!e.IsActive){ModelState.AddModelError("","حساب کاربری کارمند فعال نیست.");return View(m);}}
 ...
 if(u.EmployeeId.HasValue)claims.Add(new Claim("EmployeeId",u.EmployeeId.Value.ToString()));
