...
public class AppDbContext(DbContextOptions<AppDbContext> options):DbContext(options){
 ...
 protected override void OnModelCreating(ModelBuilder b){
  ...
  b.Entity<Employee>().HasOne(x=>x.Unit).WithMany().HasForeignKey(x=>x.UnitId).OnDelete(DeleteBehavior.Restrict);
  b.Entity<Question>().HasOne<Position>().WithMany(x=>x.Questions).HasForeignKey(x=>x.PositionId).OnDelete(DeleteBehavior.Cascade);
  ...
  b.Entity<Question>().Property(x=>x.MaxScore).HasPrecision(10,2);
